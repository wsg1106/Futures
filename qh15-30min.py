...
def csv_resample(df_15t,rule) -> pd.DataFrame:
    ...
    df_30t.reset_index(inplace=True)

    # 将Date列转换为日期时间类型，只保留年月日
    df_30t['DateDay'] = pd.to_datetime(df_30t['Date']).dt.date

    # 按照日期分组并统计行数
    date_counts = df_30t.groupby('DateDay').size().reset_index(name='count')

    # 根据条件设置temp列的值
    df_30t['temp'] = 0  # 默认为0

    # 对于行数为23的情况，设置temp为23
    df_30t.loc[df_30t['DateDay'].isin(date_counts[date_counts['count'] == 23]['DateDay']), 'temp'] = 23

    # 对于行数为15的情况，设置temp为15
    df_30t.loc[df_30t['DateDay'].isin(date_counts[date_counts['count'] == 15]['DateDay']), 'temp'] = 15
    ...
    mask = (df_30t['temp'] == 23) | (df_30t['temp'] == 15)

    # 使用groupby函数将前22行或前14行的数据每两行合并成一行
    df_30t = df_30t[mask].groupby(['DateDay', 'temp', df_30t[mask].groupby('DateDay').cumcount() // 2]).apply(merge_rows).reset_index(drop=True)

    #把df_30t的Date恢复成索引
    df_30t.set_index("Date",inplace=True)
    return df_30t
...
def lc15_resample(filepath, name, targetdir, rule) -> None:
    ...
    df_30t = csv_resample(df_15t,rule)
    df_30t.to_csv(file_object_path)
    print("数据转换已完成: " + name)
